...
static void get_regs(struct bme280_dev *dev, uint8_t reg_addr, uint8_t *data, uint8_t len) {
	dev->readReg(dev->deviceId, reg_addr, data, len);
}

/* Write registers to BME280 */
static int8_t set_regs(struct bme280_dev *dev, uint8_t reg_addr, const uint8_t *data, uint8_t len) {
	dev->writeReg(dev->deviceId, reg_addr, data, len);
}
...
static void get_calib_data(struct bme280_dev *dev) {
	uint8_t calib_data[26];
	get_regs(dev, BME280_REG_TEMP_PRESS_CALIB_DATA, calib_data, 26);
...
int8_t bme280_init(struct bme280_dev *dev) {
	...
	uint8_t chip_id;
		//set_tout_ms(10);
	get_regs(dev, BME280_REG_CHIP_ID, &chip_id, 1);
	if (chip_id == BME280_CHIP_ID) {
			u8 reg_data = (1 << 5) | (1 << 2) | BME280_POWERMODE_FORCED;
			uint8_t cmd = BME280_SOFT_RESET_COMMAND;
			dev->chip_id = chip_id;
		//set_tout_ms(10);
			set_regs(dev, BME280_REG_RESET, &cmd, 1);
			dev->delay_us(2000); 
		//set_tout_ms(10);
			get_calib_data(dev);
			/* Hardcoded settings: osr_p=1x, osr_t=1x, forced mode */
		//set_tout_ms(10);
			set_regs(dev, BME280_REG_CTRL_MEAS, &reg_data, 1);
	} else {
...
int8_t bme280_get_sensor_data(struct bme280_data *comp_data, struct bme280_dev *dev) {
	...
	get_regs(dev, BME280_REG_DATA, reg_data, 6);
